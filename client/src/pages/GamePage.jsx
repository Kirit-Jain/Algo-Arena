...
const GamePage = () => {
  ...
  const runCode = async (retryCount = 0) => {
    ...
    try {
      ...
      if (retryCount < 1)
      {
        console.log("Judge busy, retry in 4s...");
        setTimeout(() => {
          runCode(retryCount + 1);
        }, 4000);
        return;
      }

      let errorMsg = "Judge Offline";
      ...
      setJudgeOutput({ verdict: "Error", message: errorMsg });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100vw' }}>
      ...
      <div style={{ flexShrink: 0, padding: '10px', backgroundColor: '#2d2d2d', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          {joined && (
            <span style={{ color: "#4caf50", marginRight: '15px' }}>Opponent: {opponentStatus}</span>
          )}
          <LeaveRoomButton />
          <LogoutButton />
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {/* Left: Problem Description & Judge */}
        <div style={{ flex: 1, padding: '20px', backgroundColor: '#1e1e1e', color: '#ccc', display: 'flex', flexDirection: 'column', borderRight: '1px solid #444', maxWidth: '30%' }}>
          {currentProblem ? (
        ...
        {/* Center: Code Editor */}
        <div style={{ flex: 2, display: 'flex', flexDirection: 'column', borderRight: '1px solid #444' }}>
          ...
          <SubmitButton 
            onClick={() => runCode(0)} // Pass 0 for retry count
            loading={loading} 
        ...
        {/* Right: Chat Box (NEW) */}
        <ChatBox socket={socket} roomId={room} /> 
